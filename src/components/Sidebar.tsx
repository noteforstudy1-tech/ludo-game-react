...
interface SidebarProps {
  ...
  onSendChat: (text: string, name: string) => void;
  onDisconnect: () => void;
  onStartGame: (players: PlayerColor[], bots: PlayerColor[], mode: 'pass' | 'bot' | 'online') => void;
}
...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const [setupMode, setSetupMode] = useState<'pass' | 'bot' | 'online'>('pass');
  ...
  const handleStartLocal = () => {
    ...
    if (playerCount === 2) players = ['red', 'yellow']; // Opposite sides
    else if (playerCount === 3) players = ['red', 'green', 'yellow'];
    else players = ['red', 'green', 'yellow', 'blue'];
    
    let bots: PlayerColor[] = [];
    if (setupMode === 'bot') {
      bots = players.slice(1); // Everyone except red is a bot
    }
  ...
  const handleStartOnline = () => {
    ...
    let players: PlayerColor[] = [];
    if (connectedCount === 2) players = ['red', 'green']; // We map peers 1-to-1 based on colors array in network hook
            ...
            <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2">
              ...
              <div className="bg-gray-900 p-4 rounded-lg border border-gray-700 space-y-2">
                 ...
                 <div className="flex gap-2">
                    ...
                    {playerCount > 2 && <div className="w-6 h-6 rounded-full bg-ludo-green" title="Green (P2)"></div>}
                    <div className="w-6 h-6 rounded-full bg-ludo-yellow" title="Yellow"></div>
                    {playerCount === 4 && <div className="w-6 h-6 rounded-full bg-ludo-blue" title="Blue (P4)"></div>}
                 </div>
                 {setupMode === 'bot' && (
    ...
    <div className="w-full md:w-80 bg-gray-800 border-l border-gray-700 flex flex-col h-[500px] md:h-screen">
      ...
      <div className="flex-1 flex flex-col min-h-0 bg-gray-900">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          ...
          {networkState.messages.length === 0 && (
