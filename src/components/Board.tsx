...
import { isSafeSpot } from '../utils/boardMap';
...
const TokenUI = ({ token, isClickable, onClick }: { token: Token, isClickable: boolean, onClick: () => void }) => {
  const colorMap = {
...
export const Board: React.FC<BoardProps> = ({ state, onTokenClick, myColor }) => {
  const { tokens, currentTurn, diceValue, hasRolled } = state;

  const renderTokens = (cellTokens: Token[]) => {
  ...
  const renderYard = (color: PlayerColor, gridArea: string, bgClass: string, innerClass: string) => {
    const yardTokens = Object.values(tokens).filter(t => t.color === color && t.pathIndex === -1);
    const slots = [null, null, null, null];
    yardTokens.forEach((token, i) => {
      if (i < 4) slots[i] = token as any;
    });
          ...
          <div className="grid grid-cols-2 grid-rows-2 gap-4 w-full h-full max-w-[120px] max-h-[120px]">
            {slots.map((token, i) => (
              <div key={i} className={clsx("rounded-full border-2 flex items-center justify-center shadow-inner", innerClass)}>
                {token && renderTokens([token as Token])}
              </div>
            ))}
          </div>
        </div>
  ...
  for (let y = 0; y < 15; y++) {
    for (let x = 0; x < 15; x++) {
      ...
      if (isSafeSpot(x, y) && !(x === 1 && y === 6) && !(x === 8 && y === 1) && !(x === 13 && y === 8) && !(x === 6 && y === 13)) {
        content = <Star className="w-5 h-5 text-gray-400" />;
      ...
      pathCells.push(
        ...
        >
          {content && <div className="absolute inset-0 flex items-center justify-center opacity-30">{content}</div>}
